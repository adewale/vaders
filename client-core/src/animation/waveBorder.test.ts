// client-core/src/animation/waveBorder.test.ts
...
describe('BRAILLE_DENSITY', () => {
  ...
  test('density (dot count / popcount) increases monotonically', () => {
    for (let i = 1; i < BRAILLE_DENSITY.length; i++) {
      ...
      const prevPopcount = prevCode
        .toString(2)
        .split('')
        .filter((b) => b === '1').length
      const currPopcount = currCode
        .toString(2)
        .split('')
        .filter((b) => b === '1').length
...
describe('getCells', () => {
  ...
  test('no cells overlap the content area', () => {
    ...
    for (const cell of cells) {
      const insideContent =
        cell.x >= contentLeft && cell.x < contentRight && cell.y >= contentTop && cell.y < contentBottom
      // Interior ripple cells must not be in content area
      // (border cells are on the perimeter so they won't be inside content)
      if (cell.y > 0 && cell.y < config.boxHeight - 1 && cell.x > 0 && cell.x < config.boxWidth - 1) {
