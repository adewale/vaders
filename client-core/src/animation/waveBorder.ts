...
export class WaveBorderAnimation {
  ...
  constructor(config: WaveBorderConfig) {
    ...
    this.rippleFade = RIPPLE_MIN_INTENSITY ** (1 / Math.max(1, ticksToReachBorder))
  }
    ...
    for (const ripple of this.ripples) {
      ...
      for (let y = 1; y < this.config.boxHeight - 1; y++) {
        for (let x = 1; x < this.config.boxWidth - 1; x++) {
          // Skip content area
          if (x >= this.contentLeft && x < this.contentRight && y >= this.contentTop && y < this.contentBottom) {
            continue
          }
          ...
          const ringDist = Math.abs(dist - ripple.radius)
          if (ringDist < RIPPLE_RING_WIDTH) {
            const density = Math.round((1 - ringDist / RIPPLE_RING_WIDTH) * ripple.intensity * MAX_DENSITY)
            if (density > 0) {
              cells.push({
                x,
                y,
