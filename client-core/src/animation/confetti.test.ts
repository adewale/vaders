// client-core/src/animation/confetti.test.ts
...
describe('ConfettiSystem', () => {
  ...
  describe('constructor', () => {
    ...
    test('accepts custom config', () => {
      const custom = new ConfettiSystem({ width: 80, height: 24 }, { maxParticles: 50, useAscii: true })
      expect(custom).toBeDefined()
    })

    test('pre-allocates particle pool', () => {
      const custom = new ConfettiSystem({ width: 120, height: 36 }, { maxParticles: 10 })
  ...
  describe('ASCII mode', () => {
    test('uses ASCII characters when configured', () => {
      const asciiConfetti = new ConfettiSystem({ width: 120, height: 36 }, { useAscii: true })
      asciiConfetti.start()
      for (let i = 0; i < 10; i++) {
...
describe('Particle Pool: Capping and Reuse', () => {
  test('pool size never exceeds maxParticles', () => {
    ...
    const system = new ConfettiSystem({ width: 120, height: 36 }, { maxParticles, particlesPerBurst: 15 })
    system.start()
  ...
  test('pool is pre-allocated to maxParticles size at construction', () => {
    ...
    const system = new ConfettiSystem({ width: 120, height: 36 }, { maxParticles })
  ...
  test('second start resets all particles for reuse', () => {
    const system = new ConfettiSystem({ width: 120, height: 36 }, { maxParticles: 30, particlesPerBurst: 10 })
