  ...
  return (
    <box width={terminalWidth} height={terminalHeight} justifyContent="center" alignItems="center">
      ...
      {confettiParticles.map((p, i) => (
        <text key={`c-${i}`} position="absolute" top={p.y} left={p.x} fg={convertColorForTerminal(p.color, caps)}>
          {p.char}
        </text>
      ))}
      ...
      >
        ...
        {richColor ? (
          <GradientText
            ...
            colors={GRADIENT_PRESETS.victory}
        ...
        {richColor ? (
          <GradientText
            text={`Wave Reached: ${state.wave}`}
            colors={GRADIENT_PRESETS.ocean}
            fallbackColor={COLORS.ui.wave}
            richColor={richColor}
          />
        ) : (
