// client/src/animation/easing.property.test.ts
...
describe('easeOutQuad (property-based)', () => {
  ...
  it('output is in [0, 1] for all t in [0, 1]', () => {
    fc.assert(fc.property(arbUnitInterval, (t) => {
  ...
  it('is monotonically non-decreasing for t in [0, 1]', () => {
    fc.assert(fc.property(arbUnitInterval, arbUnitInterval, (a, b) => {
...
describe('easeOutElastic (property-based)', () => {
  ...
  it('output converges to 1 — final quarter is within [0.9, 1.1]', () => {
    fc.assert(fc.property(fc.double({ min: 0.75, max: 1, noNaN: true, noDefaultInfinity: true }), (t) => {
...
describe('lerp (property-based)', () => {
  it('lerp(a, a, t) = a for any t', () => {
    fc.assert(fc.property(arbFinite, arbUnitInterval, (a, t) => {
      expect(lerp(a, a, t)).toBe(a)
    }))
  })

  it('lerp(a, b, 0) = a', () => {
    fc.assert(fc.property(arbFinite, arbFinite, (a, b) => {
  ...
  it('result is between a and b for t in [0, 1] (when a <= b)', () => {
    fc.assert(fc.property(arbFinite, arbFinite, arbUnitInterval, (x, y, t) => {
...
describe('clamp (property-based)', () => {
  it('output is always within [min, max]', () => {
    fc.assert(fc.property(arbFinite, arbFinite, arbFinite, (v, x, y) => {
